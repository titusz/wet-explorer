...
export class FirstVisitNotice extends StoreElement {
  ...
  protected render() {
    return store.state.prefs.noticeDismissed
      ? nothing
      : html`<aside class="first-notice"><span><span class="wide-copy">These records are raw web text. Nobody has read, filtered or corrected them, here or at Common Crawl.</span><span class="narrow-copy">Raw web text. Nobody has read, filtered or corrected it.</span></span><button type="button" aria-label="Dismiss the raw-text notice" @click=${() => startController().preferences({ noticeDismissed: true })}>${icon("close")}</button></aside>`;
  }
}
