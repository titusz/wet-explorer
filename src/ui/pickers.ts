...
export class SegmentGrid extends StoreElement {
  ...
  protected render() {
    return html`<div class="segment-grid" role="group" aria-label="Segments">${this.segments.map(([segment, files], index) => this.cell(segment, files, index))}</div>`;
  }
...
export class FileGrid extends StoreElement {
  ...
  protected render() {
    if (!this.mobile) {
      ...
      return html`<div class="desktop-files" role="group" aria-label="Files">${rows.map((files) => html`<div class="file-grid-row"><span class="file-row-label" aria-hidden="true">${files[0]?.file.slice(-5)}</span><div class="file-grid">${files.map((file) => this.cell(file))}</div></div>`)}</div>`;
    }
    const ranges = Array.from(
      { length: Math.ceil(this.files.length / 100) },
      (_, i) => this.files.slice(i * 100, (i + 1) * 100),
    );
    return html`<div class="mobile-files" role="group" aria-label="File ranges">${ranges.map((files, i) => html`<details name="file-range" class="file-range" @toggle=${(event: Event) => this.toggleRange(event, i)}><summary><span>${files[0]?.file.slice(-5)} – ${files.at(-1)?.file.slice(-5)}</span><span aria-hidden="true">›</span></summary>${this.range === i ? html`<div class="file-grid" role="group" aria-label=${`Files ${i * 100} to ${i * 100 + files.length - 1}`}>${files.map((file) => this.cell(file))}</div>` : nothing}</details>`)}</div>`;
  }
