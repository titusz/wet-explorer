...
The private-memory and frame budgets pass locally on Windows and Ubuntu after bounded metadata sharing. Titusz approved private footprint as the counter with resident memory also reported; Linux follows Chromium's anonymous-memory-plus-swap formula and reports private mappings separately. The [completion audit](completion-audit.md) records the live Chrome/Firefox/WebKit evidence and the remaining hosted CI, deployment and actual Safari application checks.
