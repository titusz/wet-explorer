...
The private-memory and frame budgets pass locally on Windows and Ubuntu and in hosted CI after bounded metadata sharing. Titusz approved private footprint as the counter with resident memory also reported; Linux follows Chromium's anonymous-memory-plus-swap formula and reports private mappings separately. The [completion audit](completion-audit.md) records hosted verification, tagged Pages deployment, live Chrome/Firefox checks and the wide-screen layout correction. Safari-specific support is outside the release requirements. `responsive.spec.ts` covers centered, readable landing and picker layouts at five widths from 1920 through 3840 px.
